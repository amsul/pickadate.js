...
TimePicker.prototype.now = function( type, value/*, options*/ ) {
    ...
    if ( Picker._.isInteger( value ) ) {
        nowMinutes += interval * (
            value < 0 && interval * value + nowMinutes < 0 && type != 'max' ?
                value + 1 :
                value
            )
    }
...
TimePicker.prototype.parse = function( type, value, options ) {
    ...
    clock.formats.toArray( options.format ).map( function( label ) {
        ...
        var
            ...
            formatLength = formattingLabel ?
                Picker._.trigger( formattingLabel, clock, [ value, parsingObject ] ) :
    ...
    for ( item in parsingObject ) {
        ...
        if ( Picker._.isInteger(parseValue) ) {
        ...
        Picker._.group({
            ...
            item: function( loopedTime ) {
                ...
                return [
                    ...
                    'data-pick=' + loopedTime.pick + ' ' + Picker._.ariaAttr({
                        role: 'button',
                        controls: clock.$node[0].id,
                        checked: isSelected && clock.$node.val() === Picker._.trigger(
