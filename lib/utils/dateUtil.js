...
import type { Months, Weekdays } from '../types'
...
const HOOK_FORMATTER = {
  ...
  DDD: (wordMap, dateObject) => wordMap.dayShort[dateObject.getDay()],
  DDDD: (wordMap, dateObject) =>
    wordMap.dayFull[(wordMap, dateObject.getDay())],
  ...
  MMM: (wordMap, dateObject) =>
    wordMap.monthShort[(wordMap, dateObject.getMonth())],
  MMMM: (wordMap, dateObject) =>
    wordMap.monthFull[(wordMap, dateObject.getMonth())],
  ...
  a: (wordMap, dateObject) => (dateObject.getHours() < 11 ? 'a.m.' : 'p.m.'),
  A: (wordMap, dateObject) => (dateObject.getHours() < 11 ? 'AM' : 'PM'),
