...
class DatePickerWithoutApi extends React.Component<DatePickerWithoutApiProps> {
  picker = createPicker(this.props.initialState)
  render() {
...
type DatePickerWithApiProps = {
  ...
  isInputActive: boolean,
  isInputOpen: boolean,
  onChangeValue?: (formattedValue: string) => any,
}
...
class DatePickerWithApi extends React.Component<DatePickerWithApiProps> {
  ...
  unsubscribeValue = this.props.picker.subscribeToValue(formattedValue => {
  ...
  render() {
    const { picker, isInputActive, isInputOpen } = this.props
...
export default class Pickadate extends React.Component<PickadateProps> {
  ...
  unsubscribe = this.picker.subscribeToValue(formattedValue => {
    const { onChangeValue } = this.props
    if (onChangeValue) {
      onChangeValue(formattedValue)
    }
