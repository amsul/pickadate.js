...
function DatePicker( picker, settings ) {
    ...
    // Set a default date.        
    	...
    	set( 'highlight', settings.default)
...
DatePicker.prototype.nodes = function( isOpen ) {
    ...
    _.node(
        'div',
        _.node( 'button', settings.save, settings.klass.buttonSave,	
                'type=button data-save=1' + 
                ( isOpen ? '' : ' disabled' ) + ' ' +
                _.ariaAttr({ controls: calendar.$node[0].id }) ) +                
        _.node( 'button', settings.close, settings.klass.buttonClose,	
                'type=button data-close=1' + 
                ( isOpen ? '' : ' disabled' ) + ' ' +
                _.ariaAttr({ controls: calendar.$node[0].id }) ) +        
        _.node( 'button', settings.today, settings.klass.buttonToday,	
				'type=button ' + nowObject.pick +
				( isOpen ? '' : ' disabled' ) + ' ' +
				_.ariaAttr({ controls: calendar.$node[0].id }) ) +       
...
DatePicker.defaults = (function( prefix ) {
    ...
    return {
        ...
        // Saving on an outer click
        outerClickSave: false,
