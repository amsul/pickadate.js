...
import type { CreateConfig, ClassNameDynamic } from './types'
...
type Initialize = (HTMLElement, DatePickerStore, CreateConfig) => void
type Layout = (DatePickerStore, CreateConfig) => HTMLElement

const createLayout: CreateLayout = tagName => initialize => (
  ...childLayouts
) => (dateStore, config) => {
  const element = createDocumentElement({ tagName })
  initialize(element, dateStore, config)

  const children = childLayouts.map(layout => layout(dateStore, config))
...
export const createRootBox = createBoxLayout((element, dateStore, config) => {
  addAttributes(element, { className: config.className.root })
})

export const createHeaderBox = createBoxLayout((element, dateStore, config) => {
  addAttributes(element, { className: config.className.header })
})

export const createBodyBox = createBoxLayout((element, dateStore, config) => {
  addAttributes(element, { className: config.className.body })
})

export const createFooterBox = createBoxLayout((element, dateStore, config) => {
  addAttributes(element, { className: config.className.footer })
})

export const createTimeBox = createBoxLayout((element, dateStore, config) => {
  // $FlowFixMe
  const state = dateStore.getState()
  addAttributes(element, {
    className: config.className.time,
...
export const createMonthAndYearBox = createBoxLayout(
  (element, dateStore, config) => {
    addAttributes(element, {
      className: config.className.monthAndYear,
    })
    ...
    const render = () => {
      ...
      appendChildren(element, [
        createDocumentElement({
          children: config.templateHookWords.MMMM[state.view.getMonth()],
          className: config.className.monthAndYear_month,
        }),
        createDocumentElement({
          children: state.view.getFullYear(),
          className: config.className.monthAndYear_year,
        }),
      ])
    }
...
export const createPreviousButton = createButtonLayout(
  (element, dateStore, config) => {
    ...
    addAttributes(element, {
      className: config.className.button_previous,
      onMouseDown,
      onClick,
    })
...
export const createTodayButton = createButtonLayout(
  (element, dateStore, config) => {
    ...
    addAttributes(element, {
      className: config.className.button_today,
      onMouseDown,
      onClick,
    })
...
export const createNextButton = createButtonLayout(
  (element, dateStore, config) => {
    ...
    addAttributes(element, {
      className: config.className.button_next,
      onMouseDown,
      onClick,
    })
...
export const createClearButton = createButtonLayout(
  (element, dateStore, config) => {
    ...
    addAttributes(element, {
      className: config.className.button_clear,
      onMouseDown,
      onClick,
    })
...
export const createGridButton = createButtonLayout(
  (element, dateStore, config) => {
    ...
    addAttributes(element, {
      className: config.className.grid,
      onClick,
      onMouseUp: onClick,
      onKeyDown,
    })
    ...
    appendChildren(
      element,
      createGridCellElements(config, dateStore.getState())
    )
...
export const createMeridiemButton = createButtonLayout(
  (element, dateStore, config) => {
    ...
    addAttributes(element, {
      className: config.className.button_meridiem,
      onMouseDown,
      onClick,
    })
...
export const createHourInput = createBoxLayout((element, dateStore, config) => {
  addAttributes(element, { className: config.className.time_hours })

  const cycleUp = () => dateStore.cycleHourUp()
  const cycleDown = () => dateStore.cycleHourDown()

  const hourInput = createTimeInputElement(config, {
    className: config.className.time_input__hours,
    onKeyCodeUp: cycleUp,
  ...
  appendChildren(element, [
    hourInput,
    ...createTimeCounterElements(config, {
      onClickUp: cycleUp,
...
export const createMinuteInput = createBoxLayout(
  (element, dateStore, config) => {
    addAttributes(element, { className: config.className.time_minutes })

    const cycleUp = () => dateStore.cycleMinuteUp()
    const cycleDown = () => dateStore.cycleMinuteDown()

    const minuteInput = createTimeInputElement(config, {
      className: config.className.time_input__minutes,
      onKeyCodeUp: cycleUp,
      onKeyCodeDown: cycleDown,
    })
...
export const createTimeSeparator = createBoxLayout(
  (element, dateStore, config) => {
    addAttributes(element, { className: config.className.time_separator })
    element.innerHTML = ':'
  }
)
...
const createGridCellElement = (config, dateState, dateObject) => {
  ...
  const isHighlighted =
    // when first clicking, moves from 0 to selection
    // when first tabbing in, doesn't show at all
    // !!dateState.selected &&
    dateUtil.isSameDate(dateState.highlighted, dateObject)
