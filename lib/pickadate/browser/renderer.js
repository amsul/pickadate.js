...
import type { DatePickerStore, Weekdays } from '../../types'
...
type Initialize = (HTMLElement, DatePickerStore, PickerOptions) => void
type Layout = (DatePickerStore, PickerOptions) => HTMLElement
  ...
  (element, dateStore, options) => {
    ...
    const setHourInputValue = () => {
      ...
      const hours = state.selected
        ? jsUtil.padZero(state.selected.getHours(), 2)
        : ''
