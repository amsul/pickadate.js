...
shadow.Object.extend({
     ...
     */
    add: function(name, unit, comparator, collection) {
        var element = this;
        var attrValue = element.attrs[name];
        collection = collection || attrValue;
        var indexInAttr = _.indexIn(collection, unit, comparator);
        if (indexInAttr > -1) {
            return;
        }
        var insertAt = collection.length;
        collection.splice(insertAt, 0, unit);
     ...
     */
    remove: function(name, unit, comparator, collection) {
        var element = this;
        var attrValue = element.attrs[name];
        collection = collection || attrValue;
        var indexInAttr = _.indexIn(collection, unit, comparator);
        ...
        collection.splice(indexInAttr, 1);
...
shadow.Element.extend({
    ...
    create: function(options) {
        ...
        dataElement.on("set:format." + dataElement.id + " set:formatRange." + dataElement.id, function() {
            if (attrs.select) {
                attrs.value = dataElement.format(attrs.select);
            }
        });
    ...
    format: function(value, options) {
        ...
        var formatValueUnit = function(valueUnit) {
            if (formatsHash) {
                return toFormattingArray(dataElement.attrs.format, formatsHash).map(function(chunk) {
        ...
        if (dataElement.attrs.allowMultiple === true) {
            return formatMultipleUnits(formatValueUnit, dataElement.attrs.formatMultiple, dataElement.attrs.formatRange, value);
        }
        // If range values are allowed, setup the range formatter.
        if (dataElement.attrs.allowRange === true) {
            return formatRangeUnits(formatValueUnit, dataElement.attrs.formatRange, value);
        }
    ...
    parse: function(string) {
        ...
        if (dataElement.attrs.allowMultiple === true) {
            return parseMultipleUnits(parseValueUnit, dataElement.attrs.formatMultiple, dataElement.attrs.formatRange, string);
        }
        // If range values are allowed, setup the range parser.
        if (dataElement.attrs.allowRange === true) {
            return parseRangeUnits(parseValueUnit, dataElement.attrs.formatRange, string);
        }
